...
WrapInferMetaPythonFunction createOperationFactory(
  std::unordered_map<std::string, uintptr_t>& attrs_map){
  
  int64_t py_fun_ptr = attrs_map["infer_meta_fn_ptr"];
  // 这里调用 Python 的 infer_meta_fn_ptr 来做 infer_meta, 后面要将俩合并在一起
  return [=](const std::vector<paddle::dialect::IrTensor>& vec_dense_inputs) -> std::vector<IrTensor> {
    py::gil_scoped_acquire acquire;
    
    PyObject *infer_meta =
        reinterpret_cast<PyObject *>(py_fun_ptr);

    py::tuple py_args(vec_dense_inputs.size());
    size_t index = 0;
    for (const auto &ir_tensor : vec_dense_inputs) {
      py_args[index++] = py::cast(ir_tensor);
    }

    PyObject *raw_result = PyObject_CallObject(infer_meta, py_args.ptr());
    if (!raw_result) {
      PyErr_Print();
    ...
    std::vector<IrTensor> output_tensors;

    // 使用 isinstance 检查是否是 tuple
    if (py::isinstance<py::tuple>(result)) {
      py::tuple tuple_result = py::cast<py::tuple>(result);
      for (const auto &item : tuple_result) {
        output_tensors.push_back(py::cast<IrTensor>(item));
      }
    } else {
      // 如果不是 tuple，尝试作为单个 IrTensor
      output_tensors.push_back(py::cast<IrTensor>(result));
    }
...
WrapPythonFunction createOperationFactoryTensor(
  std::unordered_map<std::string, uintptr_t>& attrs_map, const std::string& key){
  int64_t py_fun_ptr = attrs_map[key];
  // namespace py = pybind11;

  // 这里调用 Python 的 infer_meta_fn_ptr 来做 infer_meta, 后面要将俩合并在一起
  return [=](std::vector<Tensor>& vec_dense_inputs) -> std::vector<Tensor> {
    ...
    VLOG(0) << "111" << " infer_meta: " << infer_meta;
    // py::tuple py_args(vec_dense_inputs.size());
    py::tuple py_args(2);
    VLOG(0) << "111.000" << "py::tuple py_args is OK";
    size_t index = 0;
    for (auto& tensor : vec_dense_inputs) {
      VLOG(0) << "111.111";
    ...
    if (py::isinstance<py::tuple>(result)) {
      py::tuple tuple_result = py::cast<py::tuple>(result);
      for (const auto &item : tuple_result) {
        output_tensors.push_back(py::cast<Tensor>(item));
      }
    } else {
      output_tensors.push_back(py::cast<Tensor>(result));
    }
    VLOG(0) << "555";
    return output_tensors;
  };
...
static PyObject *run_custom_pyop(PyObject *self,
  ...
  // paddle._C_ops._run_custom_pyop("x", inputs=["x", "y"], outputs=["z"],
  // attr=["a: int"], inplace_map={})
  ...
  // std::cout << "op_inplace_map: " << inplace_map << std::endl;





  auto xxxxxx12313 = createOperationFactoryTensor(attrs_map, "fn_ptr");
  auto infer_meta = createOperationFactory(attrs_map);



  // if (meta_info_map.at(op_name).empty()) {
  if (meta_info_map.find(op_name) == meta_info_map.end()) {
    std::cout << "We need to register this op first! " << op_name << std::endl;
    // 不存在先注册一下
    paddle::framework::RegisterPyOperator(
        op_name,
        std::move(inputs_vec),
  ...
  std::string pir_op_name = paddle::framework::kCustomPyDialectPrefix + op_name;
  if (!inplace_map.empty()) {
    pir_op_name += "_";
  }
  pir::IrContext *ctx = pir::IrContext::Instance();
  ...
  // 做 infer_meta
  std::vector<IrTensor> process_result = infer_meta(vec_dense_inputs);
  ...
  // NO NEED CHECKING, WE DO THIS IN PYTHON SIDE
  /*
  paddle::framework::CheckDefaultInferShapeDtype(
      infershape_func, inferdtype_func, vec_map[0]);
  */
  ...
  // std::vector<std::vector<int64_t>> output_shapes;
  // output_shapes.push_back(phi::vectorize(process_result.back().dims()));
  // // output_shapes.push_back({4, 5});

  // std::vector<phi::DataType> output_dtypes;
  // output_dtypes.push_back(process_result.back().dtype());
