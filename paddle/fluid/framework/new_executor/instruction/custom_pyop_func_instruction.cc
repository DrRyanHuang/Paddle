...
// #include
// "paddle/fluid/framework/new_executor/instruction/custom_kernel_instruction.h"
// 这些 PyBind 需要吗？
...
void CustomPyOpFuncInstruction::BuildCustomContext(
  ...
  const std::vector<std::string>& vec_attr_params = op_yaml_info.AttrParams(true);
...
CustomPyOpFuncInstruction::CustomPyOpFuncInstruction(
  ...
  std::cout << "CustomPyOpFuncInstruction::CustomPyOpFuncInstruction" << std::endl;

  // auto& op_inplace_map = OpMetaInfoHelper::GetInplaceMap(*custom_op_meta_);
  auto op_attributes = op->attributes();

  for( const auto& attr : op_attributes){
    VLOG(6) << "111111attr name: " << attr.first << " attr type: " << attr.second;
  }
  ...
  const auto& op_meta =
      paddle::framework::detail::GetCustomPyOpInfoByPirName(op_name);
  ...
  py_func_ptr_ = &(OpMetaInfoHelper::GetPyCustomPyOpFunction(op_meta));
  py_func_infer_meta_ptr_ = &(OpMetaInfoHelper::GetPyCustomPyOpInferMetaFunction(op_meta));
...
void CustomPyOpFuncInstruction::UpdateOutputMeta() {
  ...
  std::vector<paddle::dialect::IrTensor> output = (*py_func_infer_meta_ptr_)(vec_dense_inputs);

  VLOG(0) << "CustomPyOpFuncInstruction finish (*py_func_infer_meta_ptr_)(vec_dense_inputs);";

  for (size_t i = 0; i < cache_out_ptrs_.size(); ++i) {
    auto out_in_scope = cache_out_ptrs_.at(i);
    // update dims and dtype
    phi::DenseTensorMeta* out_meta = phi::DenseTensorUtils::GetMutableMeta(out_in_scope);
...
void CustomPyOpFuncInstruction::Run() {
  ...
  PADDLE_ENFORCE_NOT_NULL(py_func_ptr_,
                          common::errors::InvalidArgument(
                              "Custom kernel function is nullptr."));
