...
std::unique_ptr<OpYamlInfoParser> GetOpYamlInfoParser(pir::Operation* op) {
  std::cout << "wtf: std::unique_ptr<OpYamlInfoParser> GetOpYamlInfoParse"
            << std::endl;
...
void HandleForCustomPyOp(
    ...
    std::unordered_map<pir::Value, pir::Value>* map_value_pair,
    pir::Block* block) {
  std::cout << "098765456789o09876" << std::endl;
  std::cout << "HandleForCustomPyOp" << std::endl;
  // Prepare output types
  std::vector<pir::Type> op_output_types;
  ...
  VLOG(0) << "Lower custom pyop: " << op_item->name()
...
void ProcessBlock(
  ...
  for (auto iter = block->begin(); iter != block->end(); ++iter) {
    ...
    if (paddle::dialect::IsCustomPyOp(op_item)) {
      std::cout << "Hello your cute Op[op_item] is here! "
